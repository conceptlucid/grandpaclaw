...
enum GrandpaClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = GrandpaClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = GrandpaClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        GrandpaClawLogging.bootstrapIfNeeded()
        let label = GrandpaClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct GrandpaClawOSLogHandler: AppLogLevelBackedHandler {
...
struct GrandpaClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = GrandpaClawLogging.parseLabel(self.label)
