...
enum RemoteGatewayAuthIssue: Equatable {
    ...
    var body: String {
        switch self {
        case .tokenRequired:
            "Paste the token configured on the gateway host. On the gateway host, run `grandpaclaw config get gateway.auth.token`. If the gateway uses an environment variable instead, use `OPENCLAW_GATEWAY_TOKEN`."
        case .tokenMismatch:
        ...
        case .setupCodeExpired:
            "Scan or paste a fresh setup code from an already-paired GrandpaClaw client, then try again."
        case .passwordRequired:
            "This onboarding flow does not support password auth yet. Reconfigure the gateway to use token auth, then retry."
        case .pairingRequired:
            "Approve this device from an already-paired GrandpaClaw client. In your GrandpaClaw chat, run `/pair approve`, then click **Check connection** again."
        }
    }
    ...
    var footnote: String? {
        ...
        case .tokenRequired, .gatewayTokenNotConfigured:
            "No token yet? Generate one on the gateway host with `grandpaclaw doctor --generate-gateway-token`, then set it as `gateway.auth.token`."
        case .setupCodeExpired:
            nil
        case .pairingRequired:
            "If you do not have another paired GrandpaClaw client yet, approve the pending request on the gateway host with `grandpaclaw devices approve`."
    ...
    var statusMessage: String {
        ...
        case .pairingRequired:
            "Pairing required. In an already-paired GrandpaClaw client, run /pair approve, then check the connection again."
        }
    }
}
