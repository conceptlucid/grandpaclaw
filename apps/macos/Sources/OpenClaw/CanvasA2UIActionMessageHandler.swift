...
final class CanvasA2UIActionMessageHandler: NSObject, WKScriptMessageHandler {
    static let messageName = "grandpaclawCanvasA2UIAction"
    ...
    func userContentController(_: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        guard let name = GrandpaClawCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId =
        ...
        let contextJSON = GrandpaClawCanvasA2UIAction.compactJSON(userAction["context"])

        // Token-efficient and unambiguous. The agent should treat this as a UI event and (by default) update Canvas.
        let messageContext = GrandpaClawCanvasA2UIAction.AgentMessageContext(
            actionName: name,
        ...
        let text = GrandpaClawCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        Task { [weak webView] in
            ...
            await MainActor.run {
                ...
                let js = GrandpaClawCanvasA2UIAction.jsDispatchA2UIActionStatus(
                    actionId: actionId,
                    ok: result.ok,
                    error: result.error)
    ...
    // Formatting helpers live in GrandpaClawKit (`GrandpaClawCanvasA2UIAction`).
}
