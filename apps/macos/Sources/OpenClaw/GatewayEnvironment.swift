...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.grandpaclaw", category: "gateway.env")
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = GrandpaClawConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.grandpaclawExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "grandpaclaw CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "grandpaclaw@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "grandpaclaw@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "grandpaclaw@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "grandpaclaw@\(target)"])
            }

        statusHandler("Installing grandpaclaw@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed grandpaclaw@\(target)")
        } else {
