...
final class CanvasWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    init(sessionKey: String, root: URL, presentation: CanvasPresentation) throws {
        ...
        (() => {
          try {
            ...
            if (globalThis.__grandpaclawA2UIBridgeInstalled) return;
            globalThis.__grandpaclawA2UIBridgeInstalled = true;
            ...
            globalThis.addEventListener('a2uiaction', (evt) => {
              try {
                ...
                const handler = globalThis.webkit?.messageHandlers?.grandpaclawCanvasA2UIAction;
                ...
                const hasBundledA2UIHost =
                  !!globalThis.grandpaclawA2UI ||
                  !!document.querySelector('grandpaclaw-a2ui-host');
    ...
    func snapshot(to outPath: String?) async throws -> String {
        ...
        if let outPath, !outPath.isEmpty {
            ...
            path = "/tmp/grandpaclaw-canvas-\(CanvasWindowController.sanitizeSessionKey(self.sessionKey))-\(ts).png"
        }
