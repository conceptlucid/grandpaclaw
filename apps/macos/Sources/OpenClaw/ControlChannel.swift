...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: GrandpaClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.grandpaclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: GrandpaClawKit.AnyCodable]()) {
                $0[$1.key] = GrandpaClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: GrandpaClawProtocol.AnyCodable?) -> [String: GrandpaClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: GrandpaClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: GrandpaClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: GrandpaClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("grandpaclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("grandpaclaw.control.agent")
}
