...
enum RuntimeLocator {
    private static let logger = Logger(subsystem: "ai.grandpaclaw", category: "runtime")
    ...
    static func describeFailure(_ error: RuntimeResolutionError) -> String {
        ...
        case let .notFound(searchPaths):
            [
                "grandpaclaw needs Node >=22.16.0 but found no runtime.",
        ...
        case let .unsupported(kind, found, required, path, searchPaths):
            [
                ...
                "PATH searched: \(searchPaths.joined(separator: ":"))",
                "Upgrade Node and rerun grandpaclaw.",
            ].joined(separator: "\n")
