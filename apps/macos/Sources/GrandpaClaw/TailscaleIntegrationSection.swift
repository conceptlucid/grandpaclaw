...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.grandpaclaw/grandpaclaw.json. Prefer OPENCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.grandpaclaw/grandpaclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.grandpaclaw/grandpaclaw.json. Restart the gateway to apply."
        }
