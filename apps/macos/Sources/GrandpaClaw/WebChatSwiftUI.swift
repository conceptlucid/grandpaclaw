...
private let webChatSwiftLogger = Logger(subsystem: "ai.grandpaclaw", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "grandpaclaw.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: GrandpaClawChatTransport {
    func requestHistory(sessionKey: String) async throws -> GrandpaClawChatHistoryPayload {
        try await GatewayConnection.shared.chatHistory(sessionKey: sessionKey)
    }

    func listModels() async throws -> [GrandpaClawChatModelChoice] {
        do {
    ...
    func listSessions(limit: Int?) async throws -> GrandpaClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(GrandpaClawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            GrandpaClawChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return GrandpaClawChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [GrandpaClawChatAttachmentPayload]) async throws -> GrandpaClawChatSendResponse
    {
    ...
    func events() -> AsyncStream<GrandpaClawChatTransportEvent> {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> GrandpaClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                GrandpaClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    GrandpaClawChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    GrandpaClawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: GrandpaClawProtocol.ModelChoice) -> GrandpaClawChatModelChoice {
        GrandpaClawChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<GrandpaClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any GrandpaClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = GrandpaClawChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: GrandpaClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<GrandpaClawChatView>) -> NSViewController
    {
