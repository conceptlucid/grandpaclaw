...
actor CameraCaptureService {
    ...
    private let logger = Logger(subsystem: "ai.grandpaclaw", category: "camera")
        ...
        outPath: String?) async throws -> (path: String, durationMs: Int, hasAudio: Bool)
    {
        ...
        let tmpMovURL = FileManager().temporaryDirectory
            .appendingPathComponent("grandpaclaw-camera-\(UUID().uuidString).mov")
        ...
        let outputURL: URL = {
            ...
            return FileManager().temporaryDirectory
                .appendingPathComponent("grandpaclaw-camera-\(UUID().uuidString).mp4")
        }()
