...
enum CommandResolver {
    private static let projectRootDefaultsKey = "grandpaclaw.gatewayProjectRootPath"
    private static let helperName = "grandpaclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let grandpaclawEntry = root.appendingPathComponent("grandpaclaw.mjs").path
        if FileManager().isReadableFile(atPath: grandpaclawEntry) { return grandpaclawEntry }
        let binEntry = root.appendingPathComponent("bin/grandpaclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/grandpaclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let grandpaclawPaths = self.grandpaclawManagedPaths(home: home)
        if !grandpaclawPaths.isEmpty {
            extras.insert(contentsOf: grandpaclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + grandpaclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func grandpaclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".grandpaclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func grandpaclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectGrandpaClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("grandpaclaw.mjs").path,
            root.appendingPathComponent("bin/grandpaclaw.js").path,
        ]
    ...
    static func hasAnyGrandpaClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.grandpaclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func grandpaclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let grandpaclawPath = self.projectGrandpaClawExecutable(projectRoot: root) {
            return [grandpaclawPath, subcommand] + extraArgs
        }
        if let grandpaclawPath = self.grandpaclawExecutable(searchPaths: searchPaths) {
            return [grandpaclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "grandpaclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            grandpaclaw entrypoint missing (looked for dist/index.js or grandpaclaw.mjs); run pnpm build.
            """
    ...
    static func grandpaclawCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real grandpaclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/grandpaclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v grandpaclaw >/dev/null 2>&1; then
          CLI="$(command -v grandpaclaw)"
          grandpaclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/grandpaclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/grandpaclaw.mjs"
            node "$PRJ/grandpaclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/grandpaclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/grandpaclaw.js"
            node "$PRJ/bin/grandpaclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent grandpaclaw"
          pnpm --silent grandpaclaw \(quotedArgs);
        else
          echo "grandpaclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? GrandpaClawConfigFile.loadDict()
