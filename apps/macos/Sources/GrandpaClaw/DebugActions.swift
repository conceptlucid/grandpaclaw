...
enum DebugActions {
    private static let verboseDefaultsKey = "grandpaclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "grandpaclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = GrandpaClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "GrandpaClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = GrandpaClawPaths.configURL
        guard
