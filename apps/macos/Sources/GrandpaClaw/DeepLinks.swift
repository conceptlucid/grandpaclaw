...
private let deepLinkLogger = Logger(subsystem: "ai.grandpaclaw", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "GrandpaClaw is paused", message: "Unpause GrandpaClaw to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run GrandpaClaw agent?", message: body) else { return }
        }
